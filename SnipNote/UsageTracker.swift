...
struct NoteUsageParams: Encodable {
    let p_transcribed: Bool
    let p_transcription_seconds: Int
}
...
class UsageTracker {
    ...
    func trackNoteCreated(transcribed: Bool = false, transcriptionSeconds: Int = 0) async {
        do {
            let params = NoteUsageParams(
                p_transcribed: transcribed,
                p_transcription_seconds: transcriptionSeconds
            )
            try await SupabaseManager.shared.client
                .rpc("increment_note_usage", params: params)
                .execute()
        } catch {
            print("Failed to track note usage: \(error)")
        }
    }
